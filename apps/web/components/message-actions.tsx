...
export function PureMessageActions({
  ...
  const handleDownload = useCallback(async () => {
    if (!textFromParts || isExporting) return;
    ...
    try {
      const res = await fetch("/api/documents/export", {
        ...
        body: JSON.stringify({
          ...
          title: textFromParts.split("\n")[0]?.replace(/^#+\s*/, "").slice(0, 200) || "Document",
        }),
      });
  ...
  return (
    <Actions className="-ml-0.5">
      ...
      {textFromParts && textFromParts.length > 100 && (
        <Action
          data-testid="message-download-docx"
